test_that(
  "The accel_plot() returns a ggplot object for 'time' data",
  {
    data(ukb_accel)
...
test_that(
  "The accel_plot() returns a ggplot object for 'freq' data and spectral",
  {
    data(ukb_accel)
    p <- accel_plot(ukb_accel[1:100, ] |> spectral_signature(take_log = TRUE))
    expect_true(inherits(p, "gg"))
  }
)
